...
namespace Ordering.Application.Features.Orders.Commands.CheckoutOrder
{
    internal class CheckoutOrderCommandHandler : IRequestHandler<CheckoutOrderCommand, int>
    {
        ...
        {
            var email = new Email() { To = "weeshen90@gmail.com", Body = $"Order was created", Subject = $"OrderID {order.Id} " };
            try
            {
            ...
            {
                _logger.LogInformation($"Order {order.Id} Failed due to an error with the email service: {ex.Message}");
            }
        }
    }
}
